...
ovs-vsctl add-port br0 eth2 tag=10    # Access port on VLAN 10
ovs-vsctl add-port br0 eth3 tag=20    # Access port on VLAN 20
ovs-vsctl add-port br0 eth4 tag=30    # Access port on VLAN 30
ovs-vsctl set port eth5 trunks=10,20,30  # Trunk to FortiGate
```
...
# Install Libreswan:
sudo dnf install -y libreswan    # Oracle Linux
# or
sudo apt install -y libreswan    # Ubuntu/Debian
...
docker run -d --name digitara-api -p 8080:80 nginxdemos/hello
```
...
```bash
# Directory structure:
/home/bingus/Desktop/GNS3/projects/
├── FortiGate-SDWAN-Lab/
│   ├── FortiGate-SDWAN-Lab.gns3    # Main topology file
│   ├── project-files/
│   │   ├── snapshots/              # GNS3 snapshot storage
│   │   └── configs/                # Exported config backups
│   └── ...
```
...
**If you have only one FortiCloud account but need two VM trial licenses:**
...
**If your workstation cannot run GNS3 VM (insufficient RAM/CPU):**
...
**If you need more than 3 interfaces per FortiGate (e.g., dual WAN + multiple LANs):**
...
**If you need more than 3 static routes:**
...
---
*Stack research for: FortiGate SD-WAN / GNS3 Lab*
*Researched: 2026-07-15*
